...
- **בר-בקרת-וידאו מותאם (`.vctrls`)** — play/pause, seek, שעון, CC (כתוביות
  משובצות; `aria-pressed` מסונכרן), השתקה, ווליום, מסך-מלא. מוקשח לטעינה
  אמינה גם בתוך hosts מוטמעים ל-QA (ר' קומיטים "Add custom video controls
  bar…" / "Harden video controls bar and page-load reliability…").
...
## מצב נגישות (נכון ל-2026-08-26, ר' `report.md`)

דוח בקרת נגישות (WCAG 2.1 AA + ת"י 5568) מתעדכן בבדיקות חוזרות; לפי העדכון
האחרון בקובץ (2026-08-26), הסטטוס הנוכחי הוא **🔴 7 חוסמים · 🟠 5 אזהרות ·
🔵 1 ליטוש**, על פני כל 29 המסכים. חלק גדול מהתיקונים שלא דורשים שינוי-נראות
בוצע בסבב 2026-08-24; הנותרים דורשים החלטת-עיצוב חזותית (למשל ניגודיות,
אייקוני ✓/✗ נראים במקום צבע בלבד) או החלטת-תוכן/ארכיטקטורה. שלושה ממצאים
חדשים התגלו בעדכון 2026-08-26 עקב קוד חדש: משתנה CSS `--focus` לא-מוגדר
שמבטל חיווי-פוקוס על 24 כפתורי בר-הווידאו, 3 כפתורי-toggle חדשים בבר-הווידאו
בלי סנכרון `aria-pressed`, וקריינות-האודיו החדשה במסך 12 בלי תמליל/חלופת-טקסט
גלויה. **הדוח עצמו מציין** שהוא לא כלל בדיקת-דפדפן חיה (axe-core/Tab-walk/
ניגודיות-מחושבת) — יש להריץ סבב אימות דינמי לפני אישור-נגישות סופי.
